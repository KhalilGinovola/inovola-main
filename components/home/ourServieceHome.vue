<template>
  <div class="mb-[6.5rem] mt-0 md:mt-24">
    ...
    <div class="flex flex-col flex-wrap md:flex-row">
      <div
        ...
        :key="index"
        class="card mt-5 w-full md:w-1/2 xl:w-1/3 md:pe-7 h-full"
        @mouseenter="toggle(index)"
        @mouseleave="toggleOut(index)"
      >
        <nuxt-link :to="localePath(`/${service.link}`)" class="no-underline">
          <div
            class="flex flex-col items-center md:items-start min-h-[334px] text-center md:text-start w-full h-full rounded-2xl border border-border-2 hover:border-yellow border-solid bg-secondary hover:bg-yellow px-9 py-[1.625rem] md:p-8"
          >
            <div>
              <img
            ...
            <span
              class="flex justify-center sm:justify-start no-underline border-none text-primary font-semibold text-sm flex items-center"
            >
              {{ $t("shared.readMore") }}
              <img
